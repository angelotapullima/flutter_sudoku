...
class StatsScreen extends ConsumerWidget {
        ...
        body: NestedScrollView(
          ...
          body: TabBarView(
            ...
            children: [
              ...
              _buildTabContentWrapper(child: const LeaderboardView()),
            ],
          ),
        ),
      ),
  ...
  Widget _buildVitrinaHeader(bool isDark, dynamic currentTheme) {
    return Row(
  ...
  Widget _buildRecordsHeader(bool isDark) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
  ...
  Widget _buildCloudSyncCard(BuildContext context, WidgetRef ref, UserProfile userProfile, Color primaryColor, bool isDark) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(userProfile.isRegistered ? Icons.cloud_done_rounded : Icons.cloud_off_rounded, color: userProfile.isRegistered ? Colors.green : Colors.amber, size: 24),
              const SizedBox(width: 10),
              Text(userProfile.isRegistered ? 'Sincronizado con la Nube' : 'Modo Local', style: GoogleFonts.outfit(fontSize: 15, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.black87)),
              const Spacer(),
          ...
          Text(userProfile.isRegistered ? 'Cuenta activa: ${userProfile.email}\nTu progreso está respaldado de forma segura.' : 'Tu progreso solo está guardado en este dispositivo. Regístrate para competir globalmente.', style: TextStyle(fontSize: 12, color: isDark ? Colors.grey[300] : Colors.grey[700], height: 1.4)),
          const SizedBox(height: 16),
          if (!userProfile.isRegistered)
            SizedBox(width: double.infinity, child: Container(decoration: BoxDecoration(borderRadius: BorderRadius.circular(12), gradient: LinearGradient(colors: [primaryColor, primaryColor.withOpacity(0.8)])), child: ElevatedButton.icon(onPressed: () => Navigator.of(context).push(MaterialPageRoute(builder: (context) => const LoginScreen())), icon: const Icon(Icons.login_rounded, color: Colors.white, size: 18), label: const Text('INICIAR SESIÓN / REGISTRARSE', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13)), style: ElevatedButton.styleFrom(backgroundColor: Colors.transparent, shadowColor: Colors.transparent, padding: const EdgeInsets.symmetric(vertical: 14), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))))))
          else
            Row(children: [Expanded(child: OutlinedButton.icon(onPressed: () async { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Sincronizando progreso... ☁️'), duration: Duration(seconds: 1))); await ref.read(profileProvider.notifier).refreshProfileFromServer(); }, icon: const Icon(Icons.sync_rounded, size: 16), label: const Text('Sincronizar Ahora', style: TextStyle(fontSize: 12)), style: OutlinedButton.styleFrom(foregroundColor: primaryColor, side: BorderSide(color: primaryColor.withOpacity(0.5)), padding: const EdgeInsets.symmetric(vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))))), const SizedBox(width: 12), IconButton(onPressed: () { showDialog(context: context, builder: (context) => AlertDialog(shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)), backgroundColor: isDark ? const Color(0xFF222232) : Colors.white, title: const Text('¿Cerrar Sesión?', style: TextStyle(fontWeight: FontWeight.bold)), content: const Text('Tu progreso permanecerá seguro en la nube. Volverás al modo de juego local como invitado.', style: TextStyle(fontSize: 13)), actions: [TextButton(onPressed: () => Navigator.of(context).pop(), child: const Text('Cancelar', style: TextStyle(fontWeight: FontWeight.bold))), TextButton(onPressed: () async { Navigator.of(context).pop(); await ref.read(profileProvider.notifier).logout(); }, child: const Text('Cerrar Sesión', style: TextStyle(color: Colors.redAccent, fontWeight: FontWeight.bold)))])); }, icon: const Icon(Icons.logout_rounded, color: Colors.redAccent), tooltip: 'Cerrar Sesión')])
        ],
      ),
    );
  }
  ...
  Widget _buildHeaderCell(String text) {
    return Padding(padding: const EdgeInsets.symmetric(vertical: 12.0, horizontal: 8.0), child: Text(text, textAlign: TextAlign.center, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey)));
  }

  Widget _buildDataCell(String text, {bool isBold = false, Color? color}) {
    return Padding(padding: const EdgeInsets.symmetric(vertical: 14.0, horizontal: 8.0), child: Text(text, textAlign: TextAlign.center, style: TextStyle(fontSize: 13, fontWeight: isBold ? FontWeight.bold : FontWeight.normal, color: color)));
  }

  Widget _buildProfileCard(UserProfile userProfile, dynamic currentTheme, bool isDark, String userRank, String avatarEmoji) {
    return Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(borderRadius: BorderRadius.circular(28), gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: isDark ? [const Color(0xFF1E1E2E), const Color(0xFF161622)] : [Colors.white, const Color(0xFFF2F4F7)]), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.04), blurRadius: 10, offset: const Offset(0, 4))], border: Border.all(color: isDark ? Colors.white10 : Colors.grey[200]!)), child: Column(children: [Row(children: [CircleAvatar(radius: 36, backgroundColor: currentTheme.primaryColor.withOpacity(0.15), child: Text(avatarEmoji, style: const TextStyle(fontSize: 36))), const SizedBox(width: 16), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(userRank, style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.w800, color: currentTheme.primaryColor)), const SizedBox(height: 4), Text(userProfile.isRegistered ? userProfile.username : 'Jugador Invitado', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.black87)), const SizedBox(height: 2), Text('Nivel ${userProfile.level} • Racha: ${userProfile.dailyStreak} días 🔥', style: const TextStyle(fontSize: 12, color: Colors.grey, fontWeight: FontWeight.w500))]))]), const SizedBox(height: 20), Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text('Progreso de Inteligencia', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: isDark ? Colors.grey[400] : Colors.grey[600])), Text('${userProfile.xp} / ${userProfile.xpNeededForNextLevel} XP', style: GoogleFonts.shareTechMono(fontSize: 12, fontWeight: FontWeight.bold, color: currentTheme.primaryColor))]), const SizedBox(height: 8), SizedBox(height: 10, child: ClipRRect(borderRadius: BorderRadius.circular(5), child: LinearProgressIndicator(value: userProfile.progressPercentage, backgroundColor: isDark ? Colors.white10 : Colors.grey[200], valueColor: AlwaysStoppedAnimation<Color>(currentTheme.primaryColor))))]), const SizedBox(height: 16), const Divider(height: 1), const SizedBox(height: 16), Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [Column(children: [const Text('🪙', style: TextStyle(fontSize: 22)), const SizedBox(height: 4), Text('${userProfile.coins}', style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.bold)), const Text('Monedas', style: TextStyle(fontSize: 10, color: Colors.grey))]), Container(height: 30, width: 1, color: isDark ? Colors.white10 : Colors.grey[300]), Column(children: [const Text('🏆', style: TextStyle(fontSize: 22)), const SizedBox(height: 4), Text('${userProfile.unlockedAchievements.length} / ${Achievement.allAchievements.length}', style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.bold)), const Text('Medallas', style: TextStyle(fontSize: 10, color: Colors.grey))])])]));
  }

  Widget _buildAchievementTile(BuildContext context, dynamic achievement, bool isUnlocked, bool isDark, dynamic currentTheme) {
    return GestureDetector(onTap: () => _showAchievementDetail(context, achievement, isUnlocked, isDark), child: Container(decoration: BoxDecoration(color: isDark ? const Color(0xFF1E1E2E) : Colors.white, borderRadius: BorderRadius.circular(24), border: Border.all(color: isUnlocked ? currentTheme.primaryColor.withOpacity(0.35) : (isDark ? Colors.white10 : Colors.grey[200]!), width: isUnlocked ? 1.8 : 1.0), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.01), blurRadius: 4)]), padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [CircleAvatar(radius: 24, backgroundColor: isUnlocked ? currentTheme.primaryColor.withOpacity(0.12) : (isDark ? Colors.white10 : Colors.grey[100]), child: Text(achievement.icon, style: TextStyle(fontSize: 24, color: isUnlocked ? null : Colors.grey.withOpacity(0.4)))), const SizedBox(height: 8), Text(achievement.title, textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: isUnlocked ? (isDark ? Colors.white : Colors.black87) : Colors.grey)), const SizedBox(height: 4), Row(mainAxisAlignment: MainAxisAlignment.center, children: [Text(isUnlocked ? 'Desbloqueada' : 'Bloqueada', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: isUnlocked ? Colors.green : Colors.grey)), if (isUnlocked) ...[const SizedBox(width: 3), const Icon(Icons.check_circle_rounded, color: Colors.green, size: 10)]])])));
  }

  Widget _buildRecordsMotivationCard(dynamic currentTheme, bool isDark) {
    return Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: currentTheme.primaryColor.withOpacity(0.08), borderRadius: BorderRadius.circular(24), border: Border.all(color: currentTheme.primaryColor.withOpacity(0.15))), child: Row(children: [const Text('💡', style: TextStyle(fontSize: 32)), const SizedBox(width: 16), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('¡Desafía tu Lógica!', style: GoogleFonts.outfit(fontSize: 15, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.black87)), const SizedBox(height: 4), Text('Tu racha e inteligencia se sincronizan automáticamente con el ranking global. ¡Sigue jugando para escalar puestos!', style: TextStyle(fontSize: 11, color: isDark ? Colors.grey[300] : Colors.grey[700], height: 1.4))]))]));
  }
}
...
class _LeaderboardViewState extends ConsumerState<LeaderboardView> {
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (context, constraints) {
      final bool isDesktop = constraints.maxWidth > 800;
      if (isDesktop) {
        return Padding(padding: const EdgeInsets.symmetric(vertical: 24.0, horizontal: 10.0), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Container(width: 320, padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: isDark ? const Color(0xFF1E1E2E) : Colors.white, borderRadius: BorderRadius.circular(24), border: Border.all(color: isDark ? Colors.white10 : Colors.grey[200]!)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text('OPCIONES DE LIGA', style: GoogleFonts.outfit(fontWeight: FontWeight.bold, fontSize: 14, color: sudokuTheme.primaryColor, letterSpacing: 1.1)), IconButton(onPressed: _fetchLeaderboard, icon: Icon(Icons.refresh_rounded, color: sudokuTheme.primaryColor), tooltip: 'Actualizar Tabla', constraints: const BoxConstraints(), padding: EdgeInsets.zero)]), const SizedBox(height: 16), Text('Tipo de Clasificación:', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: isDark ? Colors.grey[300] : Colors.grey[700])), const SizedBox(height: 10), _buildDesktopTypeSelector(isDark, sudokuTheme.primaryColor), if (_activeTab == 'speed') ...[const SizedBox(height: 24), Text('Dificultad de Velocidad:', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: isDark ? Colors.grey[300] : Colors.grey[700])), const SizedBox(height: 10), _buildDesktopDifficultySelector(isDark)]])), const SizedBox(width: 28), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [const Text('🏆 ', style: TextStyle(fontSize: 20)), Text(_activeTab == 'level' ? 'Ranking de Maestros (Nivel)' : 'Ranking de Velocistas ($_activeDifficulty)', style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.black87))]), const SizedBox(height: 16), Expanded(child: _isLoading ? Center(child: CircularProgressIndicator(color: sudokuTheme.primaryColor)) : _errorMessage != null ? _buildErrorPlaceholder() : _leaderboardList.isEmpty ? _buildEmptyPlaceholder() : _buildLeaderboardList(isDark, sudokuTheme.primaryColor))]))]));
      }
      return Padding(padding: const EdgeInsets.all(20.0), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [_buildTypeSelectorButton(label: 'General (Nivel)', isActive: _activeTab == 'level', color: sudokuTheme.primaryColor, isDark: isDark, onTap: () { setState(() { _activeTab = 'level'; }); _fetchLeaderboard(); }), const SizedBox(width: 10), _buildTypeSelectorButton(label: 'Velocidad ⏱️', isActive: _activeTab == 'speed', color: sudokuTheme.primaryColor, isDark: isDark, onTap: () { setState(() { _activeTab = 'speed'; }); _fetchLeaderboard(); }), const Spacer(), IconButton(onPressed: _fetchLeaderboard, icon: Icon(Icons.refresh_rounded, color: sudokuTheme.primaryColor), tooltip: 'Actualizar Tabla')]), const SizedBox(height: 14), if (_activeTab == 'speed') ...[SingleChildScrollView(scrollDirection: Axis.horizontal, physics: const BouncingScrollPhysics(), child: Row(children: ['Fácil', 'Medio', 'Difícil', 'Experto'].map((diff) { final isSel = _activeDifficulty == diff; Color diffColor = diff == 'Medio' ? Colors.blueAccent : (diff == 'Difícil' ? Colors.purpleAccent : (diff == 'Experto' ? Colors.redAccent : Colors.teal)); return Padding(padding: const EdgeInsets.only(right: 8.0), child: ChoiceChip(label: Text(diff, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 11)), selected: isSel, selectedColor: diffColor.withOpacity(0.2), checkmarkColor: diffColor, labelStyle: TextStyle(color: isSel ? diffColor : (isDark ? Colors.grey[400] : Colors.grey[700])), backgroundColor: isDark ? const Color(0xFF1E1E2E) : Colors.grey[200], side: BorderSide(color: isSel ? diffColor : Colors.transparent, width: 1.2), onSelected: (val) { if (val) { setState(() { _activeDifficulty = diff; }); _fetchLeaderboard(); } })); }).toList())), const SizedBox(height: 14)], Expanded(child: _isLoading ? Center(child: CircularProgressIndicator(color: sudokuTheme.primaryColor)) : _errorMessage != null ? _buildErrorPlaceholder() : _leaderboardList.isEmpty ? _buildEmptyPlaceholder() : _buildLeaderboardList(isDark, sudokuTheme.primaryColor))]));
    });
  }
  ...
  Widget _buildDesktopTypeSelector(bool isDark, Color accentColor) { return Column(children: [_buildDesktopTypeOption(label: 'General (Nivel)', subtitle: 'Ordenado por nivel y XP', icon: '👑', isActive: _activeTab == 'level', accentColor: accentColor, isDark: isDark, onTap: () { setState(() { _activeTab = 'level'; }); _fetchLeaderboard(); }), const SizedBox(height: 12), _buildDesktopTypeOption(label: 'Velocidad ⏱️', subtitle: 'Ordenado por mejor tiempo', icon: '⚡', isActive: _activeTab == 'speed', accentColor: accentColor, isDark: isDark, onTap: () { setState(() { _activeTab = 'speed'; }); _fetchLeaderboard(); })]); }
