...
class StatsScreen extends ConsumerWidget {
                ...
                children: [
                    ...
                    child: Builder(
                      builder: (context) {
                        ...
                        return Padding(
                          ...
                          child: Column(
                            children: [
                              _buildProfileCard(userProfile, currentTheme,
                                  isDark, userRank, avatarEmoji),
                              const SizedBox(height: 20),
                              _buildCloudSyncCard(context, ref, userProfile,
                                  currentTheme.primaryColor, isDark),
                              const SizedBox(height: 24),
                              _buildVitrinaHeader(isDark, currentTheme),
                              const SizedBox(height: 12),
                              GridView.builder(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      _buildRecordsHeader(isDark,
                                          isLandscape: isLandscape),
                                      const SizedBox(height: 16),
                                      _buildStatsTable(storage, isDark,
                                          currentTheme.primaryColor,
                                          isLandscape: isLandscape),
                                    ],
                                  ),
                        ...
                        return Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 16),
                              _buildStatsTable(
                                  storage, isDark, currentTheme.primaryColor),
                  ...
                  _buildTabContentWrapper(
                      child: LeaderboardView(
                          isLandscape: isLandscape, isDesktop: isDesktop)),
                ],
              ),
            );
          },
  ...
  Widget _buildCloudSyncCard(BuildContext context, WidgetRef ref,
      UserProfile userProfile, Color primaryColor, bool isDark,
      {bool isLandscape = false}) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (!isLandscape)
            Text(
                userProfile.isRegistered
                    ? 'Cuenta activa: ${userProfile.email}\nTu progreso está respaldado de forma segura.'
                    : 'Tu progreso solo está guardado en este dispositivo. Regístrate para competir globalmente.',
                style: TextStyle(
                    fontSize: 12,
            ...
            SizedBox(
                ...
                child: Container(
                    ...
                    child: ElevatedButton.icon(
                        ...
                        label: Text(isLandscape ? 'IDENTIFICARSE' : 'INICIAR SESIÓN / REGISTRARSE',
  ...
  /// Pie de página para mostrar la versión de la app de manera dinámica
  Widget _buildVersionFooter(bool isDark) {
    return FutureBuilder<PackageInfo>(
      future: PackageInfo.fromPlatform(),
      builder: (context, snapshot) {
        String versionStr = 'Cargando versión...';
        if (snapshot.hasData && snapshot.data != null) {
          final info = snapshot.data!;
          versionStr = 'Versión ${info.version} (v${info.buildNumber})';
        }
        return Center(
          child: Text(
            versionStr,
        ...
        child: Table(
            ...
            children: [
              TableRow(
                  decoration: BoxDecoration(
                      color:
                          isDark ? const Color(0xFF252538) : Colors.grey[50]),
                  children: [
              ...
              ...difficulties.map((diff) {
                ...
                return TableRow(children: [
                  ...
                  _buildDataCell('$won'),
                  _buildDataCell(bestTimeStr,
                      color: bestTime > 0 ? accent : null)
                ]);
              })
            ]));
  }
  ...
  Widget _buildProfileCard(UserProfile userProfile, dynamic currentTheme,
      bool isDark, String userRank, String avatarEmoji,
      {bool isLandscape = false}) {
    return Container(
        ...
        child: Column(children: [
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [
            _buildCompactStat(
                '🪙', '${userProfile.coins}', 'Monedas', isLandscape),
            Container(
                height: 20,
                width: 1,
  ...
  Widget _buildCompactStat(
      String icon, String val, String label, bool isLandscape) {
    return Column(
      children: [
        Row(
          children: [
  ...
  Widget _buildAchievementTile(BuildContext context, dynamic achievement,
      bool isUnlocked, bool isDark, dynamic currentTheme,
      {bool isLandscape = false}) {
    return GestureDetector(
        onTap: () =>
            _showAchievementDetail(context, achievement, isUnlocked, isDark),
        child: Container(
            decoration: BoxDecoration(
                color: isDark ? const Color(0xFF1E1E2E) : Colors.white,
                borderRadius: BorderRadius.circular(isLandscape ? 16 : 24),
                ...
                boxShadow: [
                  BoxShadow(
                      color: Colors.black.withOpacity(0.01), blurRadius: 4)
                ]),
            padding: EdgeInsets.symmetric(
  ...
  Widget _buildRecordsMotivationCard(dynamic currentTheme, bool isDark,
    ...
    return Container(
        ...
        decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(24),
            border:
                Border.all(color: currentTheme.primaryColor.withOpacity(0.15))),
        child: Row(children: [
          ...
          Expanded(
              child: Column(
                ...
                if (!isLandscape) ...[
                  ...
                  Text(
                      'Tu racha e inteligencia se sincronizan automáticamente con el ranking global.',
                      style: TextStyle(
                          fontSize: 10,
                          color: isDark ? Colors.grey[300] : Colors.grey[700],
                          height: 1.4))
...
class LeaderboardView extends ConsumerStatefulWidget {
  ...
  const LeaderboardView(
      {super.key, this.isLandscape = false, this.isDesktop = false});
  @override
            ...
            if (_activeTab == 'speed') ...[
              SingleChildScrollView(
                ...
                child: Row(
                  children: [
                    'General',
                    ...GameDifficultyExtension.playableLabels,
                  ].map((diff) {
                    ...
                    switch (diff) {
                      case 'General':
                        ...
                        break;
                      case 'Iniciado':
                        diffColor = Colors.tealAccent;
                        break;
                      case 'Cadete':
                        diffColor = Colors.teal;
                        break;
                      case 'Explorador':
                        diffColor = Colors.cyan;
                        break;
                      case 'Viajero':
                        diffColor = Colors.blueAccent;
                        break;
                      case 'Estratega':
                        diffColor = Colors.indigoAccent;
                        break;
                      case 'Experto':
                        diffColor = Colors.purpleAccent;
                        break;
                      case 'Maestro':
                        diffColor = Colors.deepOrangeAccent;
                        break;
                    ...
                    return Padding(
                      ...
                      child: ChoiceChip(
                        ...
                        side: BorderSide(
                            color: isSel ? diffColor : Colors.transparent,
                            width: 1.2),
                        onSelected: (val) {
                          if (val) {
                            setState(() {
  ...
  Widget _buildEmptyPlaceholder() {
    return Center(
        child: Column(
          ...
          SizedBox(height: 12),
          Text('¡No hay marcas registradas aún!',
              style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
          SizedBox(height: 4),
          Text(
      ...
      children: [
        ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            String getAvatar(int level) => level <= 2
                ? '🌱'
                : (level <= 5
                    ? '🧠'
                    : (level <= 9 ? '⚡' : (level <= 14 ? '🔮' : '👑')));
